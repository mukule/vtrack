...
class UserLoginForm(AuthenticationForm):
    ...
    username = forms.CharField(
        label='username',
        widget=forms.TextInput(attrs={'placeholder': 'Staff Number or Email', 'class': 'form-control', 'id': 'inputEmailAddress'})
    )

    password = forms.CharField(
        label='',
        widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': 'Password', 'id': 'inputChoosePassword'})
    )
...
class VisitorForm(forms.ModelForm):
    class Meta:
        model = Visitor
        fields = ['full_name', 'phone', 'purpose_of_visit', 'host', 'department', 'email', 'company', 'possessions']
    ...
    purpose_of_visit = forms.CharField(
        widget=forms.TextInput(attrs={'placeholder': 'Purpose of Visit', 'class': 'form-control'}),
        label='',
    )
    ...
    company = forms.CharField(
        max_length=255,
        widget=forms.TextInput(attrs={'placeholder': 'Company (Optional)', 'class': 'form-control'}),
        label='',
        required=False,
    )
    possessions = forms.CharField(
        widget=forms.Textarea(attrs={'placeholder': 'Items in Possession (Optional)', 'class': 'form-control', 'rows':'3'}),
        label='',
        required=False,
    )
